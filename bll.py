...
class BusinessLogicLayer:
    ...
    def process_partner_summary(self, df):
        ...
        for dt in doi_tacs:
            ...
            ty_le = (
                round((lap_tb / tong_giao * 100), 1) if tong_giao > 0 else 0.0
            )
        ...
        if not summary_df.empty:
            tong_ty_le = (
                round((total_lap / total_giao * 100), 1)
                if total_giao > 0
                else 0.0
            )
            tong_row = {
    ...
    def save_progress(self, *args, **kwargs):
        ...
        chk_rai_vt = kwargs.get(
            "chk_rai_vt", args[2] if len(args) > 2 else False
        )
        chk_lap_5g = kwargs.get(
            "chk_lap_5g", args[3] if len(args) > 3 else False
        )
        ...
        chk_ps_test = kwargs.get(
            "chk_ps_test", args[5] if len(args) > 5 else False
        )
                ...
                if "Phát sóng Test" in headers:
                    ...
                    if chk_ps_test and not get_val("Phát sóng Test"):
                        self.sheet.update_cell(
                            row, col_ps_test_idx, ngay_thuc_hien
                        )

                if "Phát sóng chính thức" in headers:
                    col_ps_chinh_idx = (
                        headers.index("Phát sóng chính thức") + 1
                    )
                    if chk_ps_chinh and not get_val("Phát sóng chính thức"):
                        self.sheet.update_cell(
                            row, col_ps_chinh_idx, ngay_thuc_hien
                        )

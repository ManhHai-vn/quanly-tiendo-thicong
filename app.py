...
def login_screen():
  ...
  with col2:
    with st.form("login_form"):
      ...
      if submit:
        # Cấu hình danh khoản phân quyền cứng/hoặc linh hoạt theo yêu cầu
        # admin_agg: Quản lý toàn quyền
        # vtk_user / vcc_user: Giới hạn theo tên nhà thầu tương ứng
        users_db = {
            "admin_agg": {
                "pass": "admin123",
                "role": "admin",
...
if not st.session_state["authenticated"]:
  ...
  if current_role == "admin":
    ...
    # Đối tác nhà thầu chỉ thấy cổng báo cáo phần trạm của họ
  ...
  if page == "🛠️ 1. Cổng Báo cáo Tiến độ":
    ...
    if not df_data.empty and "Matram" in df_data.columns:
      ...
      # PHÂN QUYỀN LỌC DỮ LIỆU THEO USER
      if current_role == "partner":
        # Nhà thầu chỉ được chọn và thao tác trạm của chính họ
        st.info(
        ...
        # Admin có quyền chọn xem tất cả hoặc lọc theo từng đối tác

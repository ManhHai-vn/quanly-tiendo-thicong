...
if not st.session_state["authenticated"]:
    ...
    menu_options = ["🛠️ 1. Cổng Báo cáo Tiến độ", "📊 2. Trang Quản lý & Dashboard", "📅 3. Lịch Dự Kiến Thi Công"]
    if current_role != "admin":
        menu_options = ["🛠️ 1. Cổng Báo cáo Tiến độ", "📅 3. Lịch Dự Kiến Thi Công"]
    ...
    if page == "🛠️ 1. Cổng Báo cáo Tiến độ":
        ...
        if not df_data.empty and "Matram" in df_data.columns:
            ...
            df_hien_thi = df_hien_thi[df_hien_thi["Matram"].notna() & (df_hien_thi["Matram"].astype(str).str.strip() != "") & (df_hien_thi["Matram"].astype(str).str.lower() != "nan")]

            chon_dt = "Tất cả"
            if current_role == "partner":
                st.info(f"🔒 Tài khoản thuộc nhà thầu **{current_partner}**: Chỉ hiển thị và quản lý các trạm thuộc quyền.")
                if col_dt and col_dt in df_hien_thi.columns:
                    df_hien_thi = df_hien_thi[df_hien_thi[col_dt].astype(str).str.strip().str.upper() == current_partner.upper()]
                chon_dt = current_partner
            else:
                ds_doi_tac = ["Tất cả"] + list(df_hien_thi[col_dt].dropna().unique()) if col_dt and col_dt in df_hien_thi.columns else ["Tất cả"]
                chon_dt = st.selectbox("🏢 Lọc theo tên Đối tác (Admin):", ds_doi_tac)
                if chon_dt != "Tất cả" and col_dt and col_dt in df_hien_thi.columns:
                    df_hien_thi = df_hien_thi[df_hien_thi[col_dt] == chon_dt]

            st.markdown("---")
            st.markdown(f"### 📊 Tổng quan tiến độ thực hiện: **{str(chon_dt).upper()}**")

            tong_giao = len(df_hien_thi)
            so_nhan = count_col_done(df_hien_thi, ["Nhận VT", "Nhận thiết bị"])
            so_rai = count_col_done(df_hien_thi, ["Rải TB", "Rải thiết bị", "Rải VT"])
            so_lap = count_col_done(df_hien_thi, ["Lắp TB 5G", "Lắp đặt 5G"])
            so_bbnt = count_col_done(df_hien_thi, ["BBNT Lắp đặt", "BBNT"])

            phan_tram_lap = round((so_lap / tong_giao * 100), 1) if tong_giao > 0 else 0.0
            ...
            with col_btn_dl:
                ...
                with pd.ExcelWriter(output_partner_excel) as writer:
                    df_hien_thi.to_excel(writer, index=False, sheet_name="DanhSachTram")
                output_partner_excel.seek(0)
                
                st.download_button(
                    label="📥 Tải Excel danh sách trạm",
                    data=output_partner_excel,
                    file_name=f"DanhSach_Tram_{current_partner if current_role == 'partner' else 'TatCa'}_{datetime.now().strftime('%Y%m%d')}.xlsx",
            ...
            if not df_hien_thi.empty:
                col_phuong = "Phường xã" if "Phường xã" in df_hien_thi.columns else (df_hien_thi.columns[3] if len(df_hien_thi.columns) > 3 else df_hien_thi.columns[0])
                phuong_xa_vals = df_hien_thi[col_phuong].fillna("").astype(str)
                
                df_hien_thi["Hien_Thi_Tram"] = df_hien_thi["Matram"].astype(str) + " — " + phuong_xa_vals
                ...
                if tram_chon_full:
                    ...
                    row_filtered = df_hien_thi[df_hien_thi["Matram"].astype(str) == str(tram_chon)]
                    if not row_filtered.empty:
                        ...
                        da_rai = check_da_lam("Rải TB", "Rải thiết bị", "Rải VT", "Đã rải thiết bị đến trạm")
                        da_lap = check_da_lam("Lắp TB 5G", "Lắp đặt 5G", "Lắp thiết bị 5G")
                        da_bbnt_lap = check_da_lam("BBNT Lắp đặt", "BBNT", "Biên bản nghiệm thu lắp đặt")
                        da_ps_test = check_da_lam("Phát sóng Test", "PS Test", "Test")
                        da_ps_chinh = check_da_lam("Phát sóng chính thức", "PS Chính thức", "Phát sóng")

                        with st.form("form_bao_cao_doi_tac"):
                            st.markdown("### Tích chọn hoặc cập nhật các mốc tiến độ hoàn thành:")
                            chk_nhan_vt = st.checkbox("📦 Đối tác đã nhận vật tư", value=da_nhan)
                            chk_rai_vt = st.checkbox("🚚 Đã rải thiết bị đến trạm", value=da_rai)
                            chk_lap_5g = st.checkbox("⚡ Đã lắp đặt xong thiết bị 5G", value=da_lap)
                            chk_bbnt_lap = st.checkbox("📝 Đã ký BBNT lắp đặt", value=da_bbnt_lap)

                            chk_ps_test = False
                            chk_ps_chinh = False
                            if current_role == "admin":
                                chk_ps_test = st.checkbox("📡 Phát sóng test", value=da_ps_test)
                                chk_ps_chinh = st.checkbox("🚀 Phát sóng chính thức", value=da_ps_chinh)
...
# MỤC BÁO CÁO TIẾN ĐỘ THEO NGÀY (CHIA CỘT VCC - VTK)
                ...
                if not df_day.empty:
                    ...
                    for i, dt in enumerate(doi_tacs):
                        ...
                        with (c1 if i == 0 else c2):
                            ...
                            tong_lap = count_col_done(df_sub, ["Lắp TB 5G", "Lắp đặt 5G"])
                            st.metric(label="Tổng trạm đã lắp trong ngày", value=tong_lap)
                    ...
                    with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                        for dt in doi_tacs:
                            data_to_export[dt].to_excel(writer, sheet_name=dt, index=False)
        ...
        if not df_data.empty:
            ...
            tong_tram_giao_thuc_te = summary_df.loc[summary_df["Tên Đối Tác"] != "Tổng", "Tổng Trạm Được Giao"].sum() if not summary_df.empty else 0

            st.metric(label="📊 Tổng số trạm đã giao (VCC + VTK)", value=tong_tram_giao_thuc_te)
            st.markdown("---")
            ...
            if col_dt and col_dt in df_data.columns:
                ds_doi_tac_thuc_te = [x for x in df_data[col_dt].dropna().unique() if str(x).strip() != ""]
            ...
            if ds_doi_tac_thuc_te:
                ...
                for idx, doi_tac in enumerate(ds_doi_tac_thuc_te):
                    with cols[idx % 2]:
                        ...
                        phat_song_chinh = count_col_done(df_dt, ["Phát sóng chính thức", "PS Chính thức"])
                        ...
                        with st.container(border=True):
                            ...
                            with sub_col1:
                                ...
                                st.markdown(f"⏳ **Trạm còn phải lắp:** {tram_chua_lap}/{tong_giao}")
                            with sub_col2:
                                st.markdown(f"📝 **Đã ký BBNT lắp đặt:**<br>`{bbnt_lap}/{lap_tb}`", unsafe_allow_html=True)
                                st.markdown(f"📡 **Phát sóng test:**<br>`{phat_song_test}/{lap_tb}`", unsafe_allow_html=True)
                                st.markdown(f"🚀 **Phát sóng chính thức:**<br>`{phat_song_chinh}`", unsafe_allow_html=True)
        ...
        with st.form("form_lap_lich"):
            ...
            with col2:
                if current_role == "partner" and col_dt and col_dt in df_data.columns:
                    df_Cua_Partner = df_data[df_data[col_dt].astype(str).str.strip().str.upper() == current_partner.upper()]
                    list_ma_tram = df_Cua_Partner["Matram"].dropna().tolist() if not df_Cua_Partner.empty and "Matram" in df_Cua_Partner.columns else []
                else:
                    list_ma_tram = df_data["Matram"].dropna().tolist() if not df_data.empty and "Matram" in df_data.columns else []
                tram_du_kien = st.selectbox("Chọn trạm dự kiến:", list_ma_tram)
            with col3:
                so_doi_thi_cong = st.number_input("Số đội thi công:", min_value=1, max_value=20, value=1)

            submit_lich = st.form_submit_button("🚀 Lưu Lịch Dự Kiến")
            if submit_lich:
                ngay_str = ngay_du_kien.strftime("%d/%m/%Y")
                status, msg = bll.save_lich_du_kien(tram_du_kien, ngay_str, so_doi_thi_cong)
                if status:
                    st.success(f"🎉 {msg} - Đang cập nhật hệ thống...")
                    st.cache_data.clear()
                    st.rerun()
        ...
        if not df_data.empty:
            cols_hien_thi = [c for c in ["Matram", "Mã 5G", "Phường xã", col_dt, "Ngày dự kiến t...", "SoDoi"] if c and c in df_data.columns]
            st.dataframe(df_data[cols_hien_thi] if cols_hien_thi else df_data, use_container_width=True, hide_index=True)
